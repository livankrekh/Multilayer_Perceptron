...
class Multilayer_Perceptron(object):
	...
	def backprop(self):
		...
		for i in range(len(self.y_name)):
			tmp = np.where(self.y == self.y_name[i], 1, 0)
			y.append(pd.Series(self.res[-1][i]) - pd.Series(tmp))

		forNN.append(pd.DataFrame(y))
		...
		for i, nn in enumerate(reversed(self.NN)):
			...
			for j in range(len(z)):
				z[j] = z[j] * self.sigmoid_derivative(self.res[(i + 3) * -1].dot(self.NN[(i + 2) * -1]) + self.bias[(i + 2) * -1][j])
				nn[j] += sum(z[j]) * self.alpha
